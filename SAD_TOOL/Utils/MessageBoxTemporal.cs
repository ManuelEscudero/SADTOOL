...
namespace SAD_TOOL.Utils
{
    class MessageBoxTemporal
    {
        ...
        public MessageBoxTemporal(string texto, string titulo, int tiempo, bool contador)
        {
            ...
            if (contador)
            {
                DialogResult ResultadoMensaje = MessageBox.Show(texto + "\r\nThis message will close in " +
                    TiempoMaximo.ToString("00") + " seconds ...", titulo);
        ...
        public void Show(string texto, string titulo, int tiempo, bool contador)
        {
        ...
        public void ShowNew()
        {
            new MessageBoxTemporal(this.texto, this.titulo, this.tiempo, this.contador);
        }

        public void Start(string texto, string titulo, int tiempo, bool contador)
        {
            this.texto = texto;
            this.titulo = titulo;
            this.tiempo = tiempo;
            ...
            this.delegadoOpenTCP = new ThreadStart(this.ShowNew);
            ...
            else if (MostrarContador)
            {
                ...
                if (hndLabel != IntPtr.Zero)
                {
                    SetWindowText(hndLabel, TextoMessageBox +
                        "\r\nThis message will close in " +
                        TiempoMaximo.ToString("00") + " seconds");
                }
                else
                {
                    IntPtr hndMBox = FindWindow(null, TituloMessageBox);
